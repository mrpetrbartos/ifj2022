...
{
    std::string input = "for ($i = 0; $i < 10; $i=$i+1) { echo $i; }";

    auto result = std::list<ScannedToken>{
        ScannedToken(TOKEN_KEYWORD, {.keyword = KW_FOR}),
        ScannedToken(TOKEN_LEFT_BRACKET),
        ScannedToken(TOKEN_IDENTIFIER, {.string = "i"}),
        ScannedToken(TOKEN_ASSIGN),
        ScannedToken(TOKEN_INT, {.integer = 0}),
        ScannedToken(TOKEN_SEMICOLON),
        ScannedToken(TOKEN_IDENTIFIER, {.string = "i"}),
        ScannedToken(TOKEN_LESS_THAN),
        ScannedToken(TOKEN_INT, {.integer = 10}),
        ScannedToken(TOKEN_SEMICOLON),
        ScannedToken(TOKEN_IDENTIFIER, {.string = "i"}),
        ScannedToken(TOKEN_ASSIGN),
        ScannedToken(TOKEN_IDENTIFIER, {.string = "i"}),
...
{
    std::string input = "function bar(string $param) : string { return foo($param); }";

    auto result = std::list<ScannedToken>{
        ScannedToken(TOKEN_IDENTIFIER, {.string = "function"}),
        ScannedToken(TOKEN_IDENTIFIER, {.string = "bar"}),
        ScannedToken(TOKEN_LEFT_BRACKET),
        ScannedToken(TOKEN_IDENTIFIER, {.string = "string"}),
        ScannedToken(TOKEN_IDENTIFIER, {.string = "param"}),
        ScannedToken(TOKEN_RIGHT_BRACKET),
        ScannedToken(TOKEN_COLON),
        ScannedToken(TOKEN_IDENTIFIER, {.string = "string"}),
        ScannedToken(TOKEN_LEFT_BRACE),
        ScannedToken(TOKEN_IDENTIFIER, {.string = "return"}),
        ScannedToken(TOKEN_IDENTIFIER, {.string = "foo"}),
        ScannedToken(TOKEN_LEFT_BRACKET),
        ScannedToken(TOKEN_IDENTIFIER, {.string = "param"}),
        ScannedToken(TOKEN_RIGHT_BRACKET),
        ScannedToken(TOKEN_SEMICOLON),
